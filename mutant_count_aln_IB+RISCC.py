...
def define_option_parser():
    ...
    parser.add_option('-B', '--internal_barcode_clusters', default=None, metavar='FILE', 
                      help="Python pickle file containing internal barcode clustering data (from RISCC or IB-only PCR protocol), "
                          +"as a centroid_seq:set_of_IB_seqs dictionary (generated by ____). "
                          +"Optional - if no file provided,only identical IBs will be grouped together.")
    # MAYBE-TODO make the IB file optional, and use the old clustering-by-cassette-side-alignment method in that case?
...
def main(outfile, options):
    ...
    removed_mutant_outfile = outfile_basename + '_removed_mutants.txt'
    ...
    all_alignment_data.add_RISCC_alignment_files_to_data(options.casette_side_reads, options.genome_side_reads, 
                                                 ...
                                                 removed_mutant_file = removed_mutant_outfile, quiet = (options.verbosity_level==0))
    ...
    mutant_IB_RISCC_classes.save_dataset_files(all_alignment_data, outfile, options.verbosity_level, True, True, True, 
