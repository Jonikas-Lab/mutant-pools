...
def find_gene_by_pos(position_data, chromosome_GFF_record, position_test_function):
    """ Look up position_data in chromosome_GFF_record; return gene ID and orientation of position vs gene.

    If the location given by position_data is inside a gene listed in the chromosome_GFF_record, 
     return geneID and orientation ('sense' if position_data and gene are in the same direction, 'antisense' otherwise); 
     if location is not in a gene, return ('no_gene_found', '?').
    ...
    # go over all the genes in the chromosome record and look for one that matches the position in position_data
...
class Insertional_mutant_library_dataset():
    ...
    def find_genes_for_mutants(self, genefile, detailed_features=False, liberal_position_test=False, 
        ...
        all_reference_chromosomes = set()
        with open(genefile) as GENEFILE:
            if not detailed_features:   genefile_parsing_limits = {'gff_type': ['gene']}
            for chromosome_record in GFF.parse(GENEFILE, limit_info=genefile_parsing_limits):
                all_reference_chromosomes.add(chromosome_record.id)
                for mutant in mutants_by_chromosome[chromosome_record.id]:
                    gene_ID, orientation = find_gene_by_pos(mutant.position, chromosome_record, position_test_function)
                    mutant.gene, mutant.orientation = gene_ID, orientation
                    ...
                    if detailed_features:
                        # TODO in addition to gene ID/orientation, we want detailed gene feature (exon/intron/UTR)
                        #   Should that be implemented here, or just passed to find_gene_by_pos?
                        print "    *** WARNING: getting detailed gene features (-d option) NOT IMPLEMENTED! Skipping. ***"

        # for mutants in chromosomes that weren't listed in the genefile, use special values
        for chromosome in set(mutants_by_chromosome.keys())-all_reference_chromosomes:
