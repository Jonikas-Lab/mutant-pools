...
def find_gene_by_pos(insertion_pos, chromosome_GFF_record, location_in_gene_test, detailed_features=False):
    """ Look up insertion_pos in chromosome_GFF_record; return (gene_ID,orientation,subfeature) of insertion in gene.

    Insertion_pos is an Insertion_position instance.  If insertion_pos is inside a gene in chromosome_GFF_record, 
     return geneID, orientation ('sense' if insertion_pos and gene are in the same direction, 'antisense' otherwise), 
     and the name of the subfeature (exon/intron/UTR) the position is in (or '?' if detailed_features is False 
      or if no subfeature matched); if insertion_pos is not in a gene, return ('no_gene_found', '?', '?').

    Chromosome_GFF_record is a record generated by BCBio.GFF parser from a gff file (usually one file = multiple records). 
    Location_in_gene_test is a function that takes gene_start,gene_end,pos_start,pos_end, and returns True if pos in gene.
    """
    ...
    #   (using location_in_gene_test, which was passed as an argument - it can be more or less strict)
    for gene in chromosome_GFF_record.features:
        if location_in_gene_test(gene.location.start.position, gene.location.end.position, 
            ...
            # if we're not looking for detailed features, just return the data now
            if not detailed_features:
                return gene_ID, orientation, '?'
            # See which of the features (and subfeatures) of the gene insertion_pos is in (if any)
            # Default value is '?' if gene has no subfeatures, or 'other' if gene has them but insertion_pos isn't in any.
            gene_feature = 'other' if len(gene.sub_features)>0 else '?'
            for feature in gene.sub_features:
                # if insertion_pos is in feature, set gene_feature to its type attribute (i.e. mRNA/exon/intron/etc)
                if location_in_gene_test(feature.location.start.position, feature.location.end.position, 
                                             insertion_pos.min_position, insertion_pos.max_position):
                    gene_feature = feature.type
                    #  note that usually a gene has a single mRNA subfeature, and THAT has exon/intron/UTR subfeatures, 
                    #  so it's important to check subfeatures of the feature we found, if it has any.
                    for subfeature in feature.sub_features:
                        if location_in_gene_test(subfeature.location.start.position, subfeature.location.end.position, 
                                                     insertion_pos.min_position, insertion_pos.max_position):
...
class Insertional_mutant_library_dataset():
    ...
    def find_genes_for_mutants(self, genefile, detailed_features=False, liberal_position_test=False, 
        ...
        if liberal_position_test:   location_in_gene_test = position_test_overlap
        else:                       location_in_gene_test = position_test_contains
        ...
        for chromosome in set(mutants_by_chromosome.keys())-set(all_reference_chromosomes):
            ...
            for mutant in mutants_by_chromosome[chromosome]:
                mutant.gene = SPECIAL_GENE_CODES.chromosome_not_in_reference
                mutant.orientation = '?'
