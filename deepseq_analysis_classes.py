...
def find_gene_by_pos(chromosome, position, reference_by_chromosome, chromosomes_seen_already=set(), error_for_tests=False):
    ...
    for gene in reference_by_chromosome[chromosome].features:
        if gene.location.start.position < position < gene.location.end.position:
            return gene.id
        # TODO check the strand the gene is on, to figure out if cassette is sense or antisense to the gene, too!
...
class All_alignments_grouped_by_pos():
    ...
    def add_gene_positions_to_data(self, genefile, detailed_features=False, gene_info_file=None, known_bad_chromosomes=[]):
        ...
        for seq_group in self.data_by_position.itervalues():
            gene_ID = find_gene_by_pos(seq_group.chromosome, seq_group.position, reference_by_chromosome, 
                                    known_bad_chromosomes)
            seq_group.gene = gene_ID
...
# MAYBE-TODO rename these to something more focused on usage than on exact nature, like "Mutant_seq_group" instead of "Alignment_position_sequence_group" and "Mutants_by_pos" instead of "All_alignments_grouped_by_pos"?  Or would that be a bad idea?  Similarly, rename data_by_position to something clearer...
