...
def find_gene_by_pos_gff3(insertion_pos, chromosome_GFF_record, detailed_features=False, nearest_genes_for_intergenic=False, 
    ...
    for (gene,dist) in nearest_genes:
        ...
        if dist != 0:                           inner_feature = ''
            ...
            if len(gene.sub_features)==0:       inner_feature = 'no_mRNA'
            elif len(gene.sub_features)>1:      inner_feature = 'MULTIPLE_SPLICE_VARIANTS'
                ...
                # if insertion_pos is inside the mRNA and mRNA has no subfeatures, use 'mRNA_no_exons' as inner_feature
                    ...
                    if position_test_contains(mRNA_start, mRNA_end, ins_start, ins_end):    inner_feature = 'mRNA_no_exons'
                    else:                                                                   inner_feature = 'mRNA_edge'
                else: 
                    ...
                    # MAYBE-TODO may want to treat exons before 5'UTR or after 3'UTR specially? (not worth it, none in current file)
