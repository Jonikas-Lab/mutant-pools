...
# TODO none of this is actually done!  Just empty frameworks and copies from old code.
...
def define_option_parser():
    ...
    parser.add_option('-g', '--gene_name_reference_file', default=None, metavar='FILE', 
                      help="File to use to look up gene names/descriptions from symbols (default %default)              NOT IMPLEMENTED")
    ...
    parser.add_option('-d', '--detailed_gene_features', action="store_true", default=False,
                      help="In addition to figuring out which gene a mutant falls inside, attempt to find out what "
                          +"part of the gene it hit (UTR,intron,exon) - may take a LOT of memory! (default %default)      NOT IMPLEMENTED")
...
def run_main_function(mutant_files, gene_file, outfile, options):
    ...
    """
    all_alignment_data = deepseq_analysis_classes.All_alignments_grouped_by_pos(position_type=None)
    ...
    for mutant_file in mutant_files:
        ...
        all_alignment_data.read_from_file(mutant_file, options.assume_new_sequences)

    if not options.quiet:   all_alignment_data.print_summary()

    if options.verbose: print "parsing gene file %s... - time %s."%(gene_file, time.ctime())
    ...
    # TODO go over all mutants, I suppose by chromosome, and figure out which gene they're in (if any); keep track of totals

    # TODO print output - all this output printing is old code and will need to be rewritten!
    # TODO I guess I'll have to modify the All_alignments_grouped_by_pos.print_data function to optionally include additional fields...  Or do we even want the output file to be per-mutant, or should it be per-gene?  Or both?
    # TODO also modify the All_alignments_grouped_by_pos.print_summary function to include extra stuff like how many mutants are in genes, how many total genes were hit, how many mutants per gene, ...
