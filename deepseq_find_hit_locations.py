...
def find_gene_by_pos(chromosome, position, reference_by_chromosome, chromosomes_seen_already=[]):
    ...
    if not chromosome in reference_by_chromosome.keys():
        if not chromosome in chromosomes_seen_already:
            print('Warning: chromosome "%s" not found in gene_file data! (No further warnings will be shown)'%(chromosome))
...
def run_main_function(mutant_files, gene_file, outfile, options):
    ...
    # MAYBE-TODO what is the fraction of the genome that's covered by genes, anyway?
