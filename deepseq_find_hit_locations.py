...
def find_gene_by_pos(chromosome, position, reference_by_chromosome):
    """ If the location given by chromosome/position is inside a gene, return geneID; otherwise return "not_in_a_gene".
    Gene locations from reference_by_chromosome - a chromosome:record dict, with records generated by BCBio.GFF parser. 
    """
    return "not_in_a_gene"
    # see notes_on_GFF_parsing.txt for what reference_by_chromosome will be like
    # MAYBE-TODO may want to standardize the "not_in_a_gene" and "gene_unknown" values as global variables somewhere?
    # TODO implement!
...
def run_main_function(mutant_files, gene_file, outfile, options):
    ...
    with open(gene_file) as GENEFILE:
        ...
        # MAYBE-TODO do we even want reference_by_chromosome as a dictionary like this, or just a plain list of records?
    ...
    for mutant in all_mutants_by_pos.data_by_position.itervalues():
        mutant.gene = find_gene_by_pos(mutant.chromosome, mutant.position, reference_by_chromosome)
