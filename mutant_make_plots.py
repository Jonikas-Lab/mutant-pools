...
def read_joint_mutant_file(infile):
    ...
    sample_data = [[all_data[x][i] for i in range(8,8+N_samples)] for x in range(len(all_data))]
    annotation_data = [[all_data[x][i] for i in range(8+N_samples,len(all_data[0]))] for x in range(len(all_data))]
    ...
    return sample_data, sample_headers, mutant_data
...
######### If called directly, generate a single coverage plot and correlation plot containing data from all input files.
def main(all_data, growthrate_infiles=False, outfile_prefix='', outfile_directory='.', 
         graph_type='both', graph_scale='both', graph_format='png', print_correlation='spearman',  
         perfect_only=False, color_cassette_mutants=False, color_mutants_from_files={}):
    ...
    if outfile_prefix:  outfile_prefix += '_'
    outfile_prefix = os.path.join(outfile_directory,outfile_prefix)
    if graph_scale=="lin":     graph_scales = [('linear','_lin')]
    elif graph_scale=="log":   graph_scales = [('log','_log')]
    elif graph_scale=="both":  graph_scales = [('linear','_lin'),('log','_log')]
    # make a mutant-to-color dictionary from color_cassette_mutants and color_mutants_from_files
    mutant_to_color = get_mutant_colors(color_cassette_mutants, color_mutants_from_files)
    ...
    for (figscale,suffix) in graph_scales:
        if not graph_type=='dist':
            figname = outfile_prefix + ('growthrate_correlations' if growthrate_infiles else 'readcount_correlations')
            fig = plot_all_correlations(all_data, figscale, figname, print_correlation, mutants_to_color=mutant_to_color)
...
if __name__ == '__main__':
    ...
    parser.add_option('-t', '--graph_type', choices=['dist','corr','both'], default='both', metavar='[dist|corr|both]')
    parser.add_option('-s', '--graph_scale', choices=['lin','log','both'], default='both', metavar='[lin|log|both]')
    ...
    parser.add_option('-c', '--print_correlations', choices=['none','pearson','spearman'], default='none', 
                      metavar='none|spearman|pearson', 
                      help="Calculate and print the data correlation for each correlation graph (default %default).")
    parser.add_option('-p', '--perfect_only', action='store_true', default=False, 
                      help="count only perfectly aligned reads, instead of all reads (for counts only) (default %default)")
    ...
    # TODO how should the coloring work for the readcount distribution plots, where each sample gets a color? Either grey out all cassette dots, or make them paler or something... There should also be a -G option for other mutants to be greyed-out instead of or in addition to the cassette; other colors should be ignored.
    ...
    # parse the options for coloring mutants from specific files into a single dictionary to pass to main
    ...
    all_data = read_joint_mutant_file(infile)
    ...
    # TODO it would be nice to be able to print more detailed names/descriptions for the datasets...
    # main functionality done by "main" function
    main(all_data, options.growthrate_infiles, options.outfile_prefix, options.outfile_directory, 
