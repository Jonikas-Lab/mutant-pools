...
### histogram of the number of insertions over normalized gene length, for all genes combined

def insertions_over_gene_length(dataset, gene_positions, N_bins=20):
    ...
    gene_positions should be a gene:(start_pos,end_pos) dictionary (giving the mRNA start/end positions, or coding ones, or such)
     (like generated by gff_examine_file.gene_positions with include_chromosomes=False).
    """
    ...
    gene_binned_mutants = {gene: numpy.histogram([m.position.min_position 
                                              for m in dataset.mutants_by_gene[gene]], bins=N_bins, range=gene_positions[gene])[0] 
                           for gene in gene_positions.keys()}
    bin_totals = sum(gene_binned_mutants.values())
    bin_size = numpy.average([end-start+1 for (start,end) in gene_positions.values()])/N_bins
    mplt.bar(range(N_bins), bin_totals, align='center')
    mplt.xlim(-0.6, N_bins-0.4)
    mplt.yticks(mplt.yticks()[0], ["%.2g"%(x/bin_size/len(gene_binned_mutants)*1000) for x in mplt.yticks()[0]])
    mplt.ylabel('average mutants per kb')
    mplt.xlabel('gene sequence length (excluding UTRs) divided into %s equal sections'%N_bins)
