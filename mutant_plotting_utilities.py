...
def _get_binned_totals(dataset, gene_positions, N_bins, orientation):
    """ Get a gene:array_of_bin_mutant_counts dict giving the number of mutants in dataset for each gene in gene_positions, 
    split into N_bins along the length of the gene (values are 1D numpy arrays, not lists).
    ...
    bin_mutant_totals = sum(gene_binned_mutants.values())
    return bin_mutant_totals

def insertions_over_gene_length(dataset, gene_positions, N_bins=20, separate_sense_antisense=True, seq_info=''):
    """ Plot a histogram of the number of insertions binned by N_bins sections of normalized gene length, for all genes combined. 
    ...
    """
    # Just use numpy.histogram to get the bin info, from the DATASET.mutants_by_gene dict - for the histogram function use 
    #  N_bins and the coding sequence start/end as the range to make it ignore all mutants outside the coding sequence. 
    if separate_sense_antisense:    orientations = ['sense', 'antisense']
    else:                           orientations = ['both']
    bin_mutant_totals_dict = {orient:_get_binned_totals(dataset, gene_positions, N_bins, orient) for orient in orientations}
    total_bin_size = sum([end-start+1 for (_,start,end) in gene_positions.values()])/N_bins
    bin_mutant_densities_dict = {orient: totals/total_bin_size for (orient,totals) in bin_mutant_totals_dict.iteritems()}
    ...
    if len(bin_mutant_totals_dict)>1:
...
def insertion_density_by_feature(dataset, feature_total_lengths, separate_sense_antisense=True, show_both_for_all=True):
    ...
    Feature_lengths should be a feature_name:total_length dictionary, like generated by gff_examine_file.feature_total_lengths.
    ...
    if not separate_sense_antisense:
        # get feature mutant counts - most of the categories match feature_total_lengths, fix some details to get the right format: 
        ...
        feature_mutant_densities = [feature_mutant_counts[f]/feature_total_lengths[f] for f in features_order]
    ...
    ###  but some can't (all, intergenic), so there will be 3 sets of bars to plot: sense, antisense, and both
        ...
        for orientation in "both sense antisense".split():
            ...
            for feature in features_order:
                if feature in feature_mutant_counts:    density = feature_mutant_counts[feature]/feature_total_lengths[feature]
                else:                                   density = 0
