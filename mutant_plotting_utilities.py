...
def genes_with_N_mutants(dataset, step_size=100, max_N_mutants=3, N_mutants_colors=None, repeat_N_times=100, dataset_info='', 
    ...
    plot_xvals = range(0, len(dataset), step_size)
    for repeat in range(repeat_N_times):
        ...
        for N_mutants,gene_counts in gene_counts_by_Nmutants.items():
            ...
            mplt.plot(plot_xvals, gene_counts, '.', linewidth=0, **plot_kwargs)
...
def insertions_over_gene_length(dataset, gene_positions, gene_mappable_lengths_binned=None,
                                N_bins=20, separate_sense_antisense=True, length_type=None, seq_info='', genome_length=None):
    """ Histogram of number of insertions binned over gene length (optionally normalized to mappable length) for all genes combined.

    Dataset should be a mutant_analysis_classes.Insertional_mutant_pool_dataset instance.

    If gene_mappable_lengths_binned is given, the mutant density per bin will be calculated based on mappable bin lengths
     instead of raw bin lengths (the mutant density per bin will be calculated for each gene, then they'll be averaged together);
     gene_mappable_lengths_binned should be a gene:list_of_bin_mappable_lengths, as generated by mutant_simulations.gene_mappability
      (2nd output, run with with N_bins as the value for the split_into_bins arg.)
    ...
    If genome_length is provided, calculate the p-values (raw and FDR-adjusted) for each bin based on it, 
     using the binomial distribution and the bin/genome lengths and mutant counts; 
     genome_length should be the total genome length if gene_mappable_lengths_binned is None, 
      or else the effective genome length by the same measure as gene_mappable_lengths_binned (i.e. mappable length).
    """
    if separate_sense_antisense:    orientations = ['sense', 'antisense']
    else:                           orientations = ['both']
    # if mappable bin lengths are given, use that for bin lengths, otherwise just divide gene length by N_bins
    if gene_mappable_lengths_binned is not None:
        gene_bin_lengths = gene_mappable_lengths_binned
    else:
        gene_bin_lengths = {gene:[(end-start+1)/N_bins for _ in range(N_bins)] for (gene,(_,start,end)) in gene_positions.items()}
    bin_mutant_counts_dict = {}
    bin_mutant_densities_dict = {}
    for orient in orientations:
        # get the binned mutant counts for each gene, and the total bin counts (just take a sum of the numpy arrays)
        gene_binned_mutants = get_binned_mutant_counts(dataset, gene_positions, N_bins, orient)
        bin_mutant_counts_dict[orient] = sum(gene_binned_mutants.values())
        ...
        for gene,binned_mutants in gene_binned_mutants.items():
        ...
        #  give equal weight to each base by weighing genes by bin length (mappable or total)
    ...
    ### calculate p-value for each bin, using the binomial distribution, do FDR adjustment, and print results
    # calculate probability of a mutant falling into each bin vs elsewhere in the genome
    if genome_length is not None:
        bin_lengths = sum([numpy.array(l) for l in gene_bin_lengths.values()])
        bin_probabilities = bin_lengths/genome_length
        for orientation in orientations:
            # inputs: the number of mutants in the bin, the bin probability, and the total number of mutants.
            #  for sense/antisense orientations, divide bin_probabilities by 2
            raw_pvalues = [scipy.stats.binom_test(x=bin_count, n=len(dataset), p=bin_probability) for (bin_count, bin_probability) 
                           in zip(bin_mutant_counts_dict[orientation], bin_probabilities/len(orientations))]
            # adjust the p-values for FDR - what the total N_samples should be is not clear because they're not independent:
            #  I'm doing a total of 3*N_bins tests (sense, antisense, and both), so strictly it should be N_bins*3, 
            #  but that would be overcorrecting, since only sense/antisense are independent, so try N_bins*2 as well. 
            adjusted_pvalues_2N = numpy.array(R_stats.p_adjust(FloatVector(raw_pvalues), method='BH', n=N_bins*2))
            adjusted_pvalues_3N = numpy.array(R_stats.p_adjust(FloatVector(raw_pvalues), method='BH', n=N_bins*3))
            # Print the info:  bin mutant counts and densities, and raw and both adjusted pvalues.
            print "Bin p-values and mutant densities for %s orientation, based on %s length:"%(orientation, length_type) 
            print "   (using 2N or 3N total samples for FDR adjustment, where N=%s is the number of bins)"%N_bins
            for N, bin_density in enumerate(bin_mutant_densities_dict[orientation]):
                print " - %.3g p-value (FDR-adjusted %.3g-%.3g) - bin %s, mutant density %.3g, raw mutant count %s, length %s"%(
                    raw_pvalues[N], adjusted_pvalues_2N[N], adjusted_pvalues_3N[N], N+1, bin_density, 
                    bin_mutant_counts_dict[orientation][N], bin_lengths[N])
    ### make the plot
    ...
    # MAYBE-TODO integrate the p-values into the plot?  Easier done by hand, really, for now.
    ...
    # specify length type in ylabel; if not given, set to TOTAL if gene_mappable_lengths_binned wasn't given; otherwise we can't know
    if length_type is None:
        if gene_mappable_lengths_binned is None:    length_type = ' TOTAL' 
        else:                                       length_type = '' 
...
def insertion_density_by_feature(dataset, feature_lengths, separate_sense_antisense=True, show_both_for_all=True, 
    ...
    Also calculate the p-values for each feature, using the binomial distribution and the feature/genome lengths and mutant counts.
    """
    ...
    if not separate_sense_antisense:
        ...
        feature_mutant_counts_all['both'] = feature_mutant_counts
    ...
    ###  but some can't (all, intergenic), so there will be 3 sets of bars to plot: sense, antisense, and both
        ...
        for orientation in "both sense antisense".split():
            ...
            feature_mutant_counts_all[orientation] = feature_mutant_counts
    ...
    mplt.ylabel('average mutants per kb\nof%s feature length'%(' '+feature_length_type if feature_length_type else ''))
    mplt.yticks(mplt.yticks()[0], [x*1000 for x in mplt.yticks()[0]])
    ### calculate p-value for each feature, using the binomial distribution, do FDR adjustment, and print results
    # calculate probability of a mutant falling into each feature vs elsewhere in the genome
    feature_probabilities = {feature: length/feature_lengths['all'] for (feature,length) in feature_lengths.items()}
    assert feature_probabilities['all'] == 1
    for orientation, feature_mutant_counts in feature_mutant_counts_all.items():
        curr_features_order = [f for f in features_order if f in feature_mutant_counts and not f=='all']
        # inputs: the number of mutants in the feature, the feature probability, and the total number of mutants.
        #  for sense/antisense orientations, divide feature_probabilities by 2
        raw_pvalues = [scipy.stats.binom_test(x=feature_mutant_counts[feature], n=len(dataset), 
                                              p=feature_probabilities[feature]/(1 if orientation=='both' else 2)) 
                       for feature in curr_features_order]
        # adjust the p-values for FDR - what the total N_samples should be is not clear because they're not independent, 
        #  both because sense+antisense = both and because all the features add up to gene etc; maybe go with 
        #  2*N, where N is the length of "interesting" features (excluding "all"), and 2 for sense/antisense...
        N_samples_for_FDR = (len(features_order)-1) * 2
        adjusted_pvalues = numpy.array(R_stats.p_adjust(FloatVector(raw_pvalues), method='BH', n=N_samples_for_FDR))
        # Print the info:  feature mutant counts and densities, and raw and both adjusted pvalues.
        print "Feature p-values and mutant densities for %s orientation, based on %s length:"%(orientation, feature_length_type) 
        print "   (using %s total samples for FDR adjustment)"%N_samples_for_FDR
        for N, feature in enumerate(curr_features_order):
            print " - %s: %.3g p-value (FDR-adjusted about %.3g) - mutant count %s, length %s, density %.3g"%(
                feature, raw_pvalues[N], adjusted_pvalues[N], feature_mutant_counts[feature], feature_lengths[feature], 
                feature_mutant_counts[feature]/feature_lengths[feature])
