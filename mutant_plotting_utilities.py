...
def insertions_over_gene_length(dataset, gene_positions, N_bins=20, seq_info=''):
    ...
    for gene,(strand,start,end) in gene_positions.iteritems():
        mutant_positions = [m.position.min_position-start for m in dataset.mutants_by_gene[gene]]
        if strand=='-':
    ...
    bin_mutant_densities = [bin_mutant_total/total_bin_size for bin_mutant_total in bin_mutant_totals]
    mplt.bar(range(N_bins), bin_mutant_densities, align='center')
