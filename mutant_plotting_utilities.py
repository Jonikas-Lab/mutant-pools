...
def insertions_over_gene_length(dataset, gene_positions, gene_mappable_lengths_binned=None, count_all_genes_equal=False, 
                                N_bins=20, separate_sense_antisense=True, seq_info=''):
    ...
    After the mutant densities per bin for each gene are calculated, they can be averaged in one of two ways:
     - if count_all_genes_equal, give equal weight to each GENE by just averaging the values 
        (so if a 2kb gene has counts [1,1], and an 18kb gene has counts [0,0], the average will be 0.5 mutant per kb)
     - otherwise give equal weight to each BASE by weighing the genes by bin length
        (so if a 2kb gene has counts [1,1], and an 18kb gene has counts [0,0], the average will be 0.1 mutant per kb)
    ...
    for orient in orientations:
        # get the binned mutant counts for each gene
        ...
        for gene,binned_mutants in gene_binned_mutants.items():
            # try just doing numpy array division (faster)
            try:
                gene_mutant_densities[gene] = binned_mutants/gene_bin_lengths[gene]
            # if some bin lengths are 0, this is a problem!  In that case for average-by-gene the result will be undefined
            #   (unless I write something to skip that gene in the averaging, but that would be an extra complication), 
            #  and for average-by-base I can put whatever value I want for the 0-length bin, 
            #   since it'll be used with weight 0 in the average
            except ZeroDivisionError:
                pass
                # TODO implement something special here? Is that even worth it?  Using the average-by-gene option seems like an increasingly bad idea...
        # average all the mutant bin densities together, using numpy.average over dimension 0 of the list of all bin density arrays:
        #  either give equal weight to each gene (by simply counting each gene as 1)
        if count_all_genes_equal:
            bin_mutant_densities_dict[orient] = numpy.average(gene_mutant_densities.values(), 0)
        #  or give equal weight to each BASE (by weighing genes by bin length - mappable if given, otherwise total)
        else:
    ...
    # TODO implement calculating p-values!  Based on either mappable bin lengths, or just raw ones, whichever we have.
    ...
    for N,orientation in enumerate(orientations):
        ...
        mplt.bar([x + N*bar_width for x in range(N_bins)], bin_mutant_densities, bar_width, label=orientation, 
                 align='center', color=('magenta' if orientation=='antisense' else 'green'), edgecolor='none')
...
def insertion_density_by_feature(dataset, feature_lengths, separate_sense_antisense=True, show_both_for_all=True, 
    ...
    Ignores insertions on feature boundaries (usually <1%).
    """
    ...
    ###  but some can't (all, intergenic), so there will be 3 sets of bars to plot: sense, antisense, and both
        ...
        colors = {'both': 'blue', 'sense': 'green', 'antisense': 'magenta'}
        ...
        for orientation in "both sense antisense".split():
            ...
            mplt.bar(bar_positions, density_list, width, color=colors[orientation], label=orientation, 
    ...
    # TODO implement calculating p-values!  Based on either mappable feature lengths, or just raw ones, whichevver we have.
