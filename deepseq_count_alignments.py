...
def run_main_function(infile, outfile, options):
    ...
    # parse preprocessing/alignment metadata file to get discarded read count
    if options.input_metadata_file is 'NONE':
        discarded_read_count = 'unknown'
    else:
    ...
    with open(outfile,'w') as OUTFILE:
        ...
        if options.add_summary_to_file:
            ...
            OUTFILE.write('# '+discarded_read_info+'\n')
        if options.header_level==2:
